...
const QueryView = (props) => {
  ...
  const queries = queryItems.map((e, index) => {
    return (
      <>
        <Link to={`/query/${e?.id}`}>
          <div className="grid grid-cols-[10%,20%,55%,15%]  border-solid  border-b-2 border-black overflow-auto">
    ...
    <>
      <div>
        <div className="flex justify-center">
          <div className=" top-44 left-80 absolute h-96 w-3/4 bg-blue-600 z-10">
          <div className="grid grid-cols-[10%,20%,55%,15%]  border-solid border-y-2 border-b-2 border-black overflow-auto">
      <div className="border-r-2 border-solid border-black"> S No. </div>
        <div className="border-r-2 border-solid border-black"> Type of Query </div>
